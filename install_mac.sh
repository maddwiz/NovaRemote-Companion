...
if [[ -f "${CONFIG_FILE}" ]] && grep -q '^export CODEXREMOTE_TOKEN=' "${CONFIG_FILE}"; then
  ...
  BIND_HOST="${CODEXREMOTE_BIND_HOST:-127.0.0.1}"
  BIND_PORT="${CODEXREMOTE_BIND_PORT:-8787}"
  TMUX_BIN="${CODEXREMOTE_TMUX_BIN:-tmux}"
  EXISTING_CODEX_BIN="${CODEXREMOTE_CODEX_BIN:-}"
  EXISTING_CODEX_ARGS="${CODEXREMOTE_CODEX_ARGS:-exec --dangerously-bypass-approvals-and-sandbox}"
  DEFAULT_CWD="${CODEXREMOTE_DEFAULT_CWD:-${HOME}}"
  AUDIT_LOG="${CODEXREMOTE_AUDIT_LOG:-${CONFIG_DIR}/audit.log}"
  NOVAADAPT_ENABLED="${CODEXREMOTE_NOVAADAPT_ENABLED:-false}"
  NOVAADAPT_BRIDGE_URL="${CODEXREMOTE_NOVAADAPT_BRIDGE_URL:-http://127.0.0.1:9797}"
  NOVAADAPT_BRIDGE_TOKEN="${CODEXREMOTE_NOVAADAPT_BRIDGE_TOKEN:-}"
  NOVAADAPT_TIMEOUT_SECONDS="${CODEXREMOTE_NOVAADAPT_TIMEOUT_SECONDS:-15}"
  NOVASPINE_URL="${CODEXREMOTE_NOVASPINE_URL:-}"
  NOVASPINE_TOKEN="${CODEXREMOTE_NOVASPINE_TOKEN:-}"
else
...
PY
  ...
  TMUX_BIN="tmux"
  EXISTING_CODEX_BIN=""
  EXISTING_CODEX_ARGS="exec --dangerously-bypass-approvals-and-sandbox"
  DEFAULT_CWD="${HOME}"
  AUDIT_LOG="${CONFIG_DIR}/audit.log"
  NOVAADAPT_ENABLED="false"
  NOVAADAPT_BRIDGE_URL="http://127.0.0.1:9797"
  NOVAADAPT_BRIDGE_TOKEN=""
  NOVAADAPT_TIMEOUT_SECONDS="15"
  NOVASPINE_URL=""
  NOVASPINE_TOKEN=""
fi
...
if [[ -z "${CODEX_BIN}" ]]; then
  CODEX_BIN="${EXISTING_CODEX_BIN:-codex}"
fi

cat > "${CONFIG_FILE}" <<CFG
export CODEXREMOTE_TOKEN="${TOKEN}"
export CODEXREMOTE_BIND_HOST="${BIND_HOST}"
export CODEXREMOTE_BIND_PORT="${BIND_PORT}"
export CODEXREMOTE_TMUX_BIN="${TMUX_BIN}"
export CODEXREMOTE_CODEX_BIN="${CODEX_BIN}"
export CODEXREMOTE_CODEX_ARGS="${EXISTING_CODEX_ARGS}"
export CODEXREMOTE_DEFAULT_CWD="${DEFAULT_CWD}"
export CODEXREMOTE_AUDIT_LOG="${AUDIT_LOG}"
export CODEXREMOTE_NOVAADAPT_ENABLED="${NOVAADAPT_ENABLED}"
export CODEXREMOTE_NOVAADAPT_BRIDGE_URL="${NOVAADAPT_BRIDGE_URL}"
export CODEXREMOTE_NOVAADAPT_BRIDGE_TOKEN="${NOVAADAPT_BRIDGE_TOKEN}"
export CODEXREMOTE_NOVAADAPT_TIMEOUT_SECONDS="${NOVAADAPT_TIMEOUT_SECONDS}"
export CODEXREMOTE_NOVASPINE_URL="${NOVASPINE_URL}"
export CODEXREMOTE_NOVASPINE_TOKEN="${NOVASPINE_TOKEN}"
CFG
